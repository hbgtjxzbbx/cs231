...
class ThreeLayerConvNet(object):
    ...
    def loss(self, X, y=None):
        ...
        loss_reg = 0.5 * self.reg * sum(np.sum(W*W) for W in [W1, W2, W3])
        loss = loss_data + loss_reg
        dar, dW3, db3 = affine_backward(dscores, cache_scores)
        dcrp, dW2, db2 = affine_relu_backward(dar, cache_ar)
        _, dW1, db1 = conv_relu_pool_backward(dcrp, cache_crp)
        dW1 += self.reg * W1
        dW2 += self.reg * W2
        dW3 += self.reg * W3
        grads['W1'] = dW1
