...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        x_normalized = (x - sample_mean)* istd
        out = gamma * x_normalized + beta
        cache = (x_normalized, gamma, istd)
        running_mean = momentum * running_mean + (1 - momentum) * sample_mean
        running_var = momentum * running_var + (1 - momentum) * sample_var
...
def batchnorm_backward(dout, cache):
    ...
    x_normalized, gamma, istd = cache
    ###########################################################################
    N, D = x_normalized.shape
    dgamma = np.sum(dout * x_normalized, axis=0, keepdims=True)
    dbeta = np.sum(dout, axis=0, keepdims=True)
    dx = gamma * istd / N * (N * dout - dgamma * x_normalized - dbeta)
...
def batchnorm_forward_old(x, gamma, beta, bn_param):
    mode = bn_param['mode']
    eps = bn_param.get('eps', 1e-5)
    momentum = bn_param.get('momentum', 0.9)
...
def batchnorm_backward_old(dout, cache):
...
def conv_backward_naive(dout, cache):
    ...
    for i in range(N):  # ith image
        for f in range(F):  # fth filter
            for j in range(H_new):
                for k in range(W_new):
                    windows= x_padded[i,:, j*s:j*s+HH, k*s:k*s+WW]
                    db[f] += dout[i,f,j,k]
                    dw[f] += dout[i,f,j,k] * windows
                    dx_padded[i,:, j*s:j*s+HH, k*s:k*s+WW] += dout[i,f,j,k] * w[f]
...
def max_pool_forward_naive(x, pool_param):
    ...
    assert (H-ph)%s ==0, "not rounded"
    assert (W-pw)%s ==0, " not rounded"
    H_new = 1 + (H-ph)//s
    W_new = 1 + (W-pw)//s
        ...
        for f in range(C):
            for h in range(H_new):
                for w in range(W_new):
                    windows = x[i,f,s*h:s*h+ph,s*w:s*w+pw]
                    out[i, f ,h, w] = np.max(windows)
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(N):
        for f in range(C):
            for h in range(H_new):
                for w in range(W_new):
                    windows = x[i, f, h*s:h*s+ph, w*s: w*s+pw]
                    m =np.max(windows)
                    dx[i, f, h * s:h * s + ph, w * s: w * s + pw] = (windows==m) *dout[i, f, h, w]
    return dx
