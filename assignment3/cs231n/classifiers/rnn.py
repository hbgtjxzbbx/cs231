...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        ############################################################################
        imf2hid, imf2hid_cache = affine_forward(features, W_proj, b_proj)
        ...
        _, grads['W_proj'],grads['b_proj'] = affine_backward(dimf2hid, imf2hid_cache)
    ...
    def sample(self, features, max_length=30):
        ...
        words = init_word[:, None]
        for i in range(max_length):
            word_vectors, _ = word_embedding_forward(words, W_embed)
            if self.cell_type == 'rnn':
                prev_h, _ = rnn_step_forward(word_vectors, prev_h, Wx, Wh, b)
            else:
                #print(word_vectors.shape, prev_h.shape, prev_c.shape, Wx.shape, Wh.shape)
                prev_h, prev_c, _ = lstm_step_forward(word_vectors,prev_h, prev_c, Wx, Wh, b)
            h = prev_h.reshape(prev_h.shape[0],1,prev_h.shape[1])
            scores, _ = temporal_affine_forward(h, W_vocab, b_vocab)
            words = np.argmax(scores, axis =2)
            captions[:,i] = np.squeeze(words)
        print(captions)
        # V, W = W_embed.shape
        ...
        #     else:
        #         print(word_vectors.shape,h.shape, c.shape, Wx.shape, Wh.shape)
